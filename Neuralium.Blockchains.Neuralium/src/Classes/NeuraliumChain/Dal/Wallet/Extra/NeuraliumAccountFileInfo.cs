...
namespace Neuralium.Blockchains.Neuralium.Classes.NeuraliumChain.Dal.Wallet.Extra {
	...
	public class NeuraliumAccountFileInfo : AccountFileInfo, INeuraliumAccountFileInfo {
		...
		public override async Task Load(LockContext lockContext) {
			...
			await this.WalletTimelineFileInfo.Load(lockContext).ConfigureAwait(false);
		}
