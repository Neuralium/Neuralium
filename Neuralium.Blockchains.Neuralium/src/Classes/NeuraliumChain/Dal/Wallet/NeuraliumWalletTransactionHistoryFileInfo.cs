...
namespace Neuralium.Blockchains.Neuralium.Classes.NeuraliumChain.Dal.Wallet {
	...
	public class NeuraliumWalletTransactionHistoryFileInfo : WalletTransactionHistoryFileInfo<NeuraliumWalletTransactionHistory>, INeuraliumWalletTransactionHistoryFileInfo {
		...
		public async Task UpdateTransactionTip(TransactionId transactionId, decimal tip, LockContext lockContext) {
			using(LockHandle handle = await this.locker.LockAsync(lockContext).ConfigureAwait(false)) {
				await this.RunDbOperation((litedbDal, lc) => {
					if(litedbDal.CollectionExists<NeuraliumWalletTransactionHistory>()) {

						NeuraliumWalletTransactionHistory entry = litedbDal.GetOne<NeuraliumWalletTransactionHistory>(k => k.TransactionId == transactionId.ToString());
		...
		public async Task<(decimal debit, decimal credit, decimal tip)> GetPendingTransactionAmounts(LockContext lockContext) {
			...
			(List<(decimal amount, decimal tip)> debits, List<(decimal amount, decimal tip)> credits) = await this.RunQueryDbOperation(async (litedbDal, lc) => {

				if(litedbDal.CollectionExists<NeuraliumWalletTransactionHistory>()) {
					var pendingtransactions = litedbDal.Get<NeuraliumWalletTransactionHistory, (decimal amount, decimal tip, Enums.BookkeepingTypes bookkeepingType)>(t => t.Status == WalletTransactionHistory.TransactionStatuses.Dispatched && t.Local, t => (t.Amount, t.Tip, t.BookkeepingType));
