...
namespace Neuralium.Blockchains.Neuralium.Classes.NeuraliumChain.Dal.Wallet {
	...
	public class NeuraliumWalletTransactionCacheFileInfo : WalletTransactionCacheFileInfo<NeuraliumWalletTransactionCache> {
		...
		public async Task<(decimal debit, decimal credit, decimal tip)> GetTransactionAmounts(LockContext lockContext) {
			...
			List<Tuple<decimal, decimal>> debits = await this.RunQueryDbOperation(async (litedbDal, lc) => {
			...
			List<Tuple<decimal, decimal>> credits = await this.RunQueryDbOperation(async (litedbDal, lc) => {
				if(collectionExists) {
